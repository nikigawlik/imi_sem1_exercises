...
{
    private static final int MAX_LEVELS = 105;
    private static final int MAX_MOVES = 999; 
    
    private UserInfo myUserInfo;
    private int topScore;
    
    /**
     * The highscore map with the level number as a key and the Highscore as a value.
     */
    ...
    public void setHighscore(Highscore highscore)
    {
        ...
        if (highscore.getFirstEntry().getName().equals(myUserInfo.getUserName())) 
        {
            ...
            saveToMyInfo(highscore.getLevelNumber(), highscore.getFirstEntry().getMoves());
        } 
        else if (highscore.getSecondEntry().getName().equals(myUserInfo.getUserName())) 
        {
            ...
            saveToMyInfo(highscore.getLevelNumber(), highscore.getSecondEntry().getMoves());
        }
        else if (highscore.getThirdEntry().getName().equals(myUserInfo.getUserName())) 
        {
            ...
            saveToMyInfo(highscore.getLevelNumber(), highscore.getThirdEntry().getMoves());
        }
    ...
    private void initHighscoresFromUserInfos() 
    {
        ...
        // Get the top UserInfos: The maximum would be 315 (3 places in 105 levels), but
        // it is very unlikely that there are 315 distinct players in the highscore of
    ...
    private void saveToMyInfo(int levelNumber, int moves)
    {
        ...
        else if (levelNumber <= 105)
        {
            ...
            // get and decode
            int[] userInfoInts = decodeString(myUserInfo.getString(userInfoIndex));
            userInfoInts[arrayIndex] = moves;
    ...
    {
        if (dec.length > 3) throw new IllegalArgumentException();
        
        int result = 1000000000; // add 1 billion, otherwise leading 0s will not be visible
    ...
    {
        if (dec.length > 15) throw new IllegalArgumentException();
