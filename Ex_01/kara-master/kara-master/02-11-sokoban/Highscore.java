...
public class Highscore
{
    ...
    public int addHighscoreEntry(String playerName, int moves)
    {
        ...
        if (entries.size() < 3 || isBetterThan(moves, entries.get(2).getMoves()))
        {
            // is in top 3 --> add
    ...
    {
        // less than 1 indicates that it is not a real entry --> they are always considered as beeing worse
    ...
    /**
     * Inner class for a highscore entry with name of the player and number of moves.
     * The Highscore.Entry implements Comparable so that it can be sorted by number of 
     * moves.
     */
