...
public class FileHighscore extends HighscoreManager 
{
    ...
    private Map<Integer, Highscore> parseHighscoresFromFile()
    {
        ...
        try
        {
            ...
            try 
            {
                while ((line = input.readLine()) != null)
                {
                    if (line.startsWith("Level:"))
                    {
                        ...
                        if (strs.length == 7)
                        {
                            int levelNumber = Integer.parseInt(strs[0].substring("Level:".length()).trim());
                            Highscore highscore = new Highscore(levelNumber);
                            highscore.addHighscoreEntry(strs[1], Integer.parseInt(strs[2]));
                            highscore.addHighscoreEntry(strs[3], Integer.parseInt(strs[4]));
                            highscore.addHighscoreEntry(strs[5], Integer.parseInt(strs[6]));
            ...
            System.out.println("ERROR: Could not load highscore from file: " + HIGHSCORE_FILE);
            ex.printStackTrace();
        }
    ...
    public void writeHighscoresToFile(Map<Integer, Highscore> highscores)
    {
        try
        {
            ...
            System.out.println("ERROR: Could not save highscore to file: " + HIGHSCORE_FILE);
            ex.printStackTrace();
        }
    }
    ...
    private String toHighscoreFileString(Highscore highscore)
    {
        ...
        buf.append(";" + highscore.getFirstEntry().getName() + ";" + highscore.getFirstEntry().getMoves());
        buf.append(";" + highscore.getSecondEntry().getName() + ";" + highscore.getSecondEntry().getMoves());
        buf.append(";" + highscore.getThirdEntry().getName() + ";" + highscore.getThirdEntry().getMoves());
        return buf.toString();
    }
}
