...
public class LevelCompleteState extends ScreenState 
{
    ...
    {
        Label levelCompleteLabel = new Label("Level " + gameScreen.getCurrentLevelNumber() + " Complete!", 400, 50, GameScreen.FONT_XL_BOLD);
        ...
        {
            initHighscore();
            gameScreen.addObject(levelCompleteLabel, GameScreen.WIDTH_IN_CELLS / 2, 3);
            gameScreen.addObject(nextLevelButton, GameScreen.WIDTH_IN_CELLS / 2, 13);
            gameScreen.addObject(retryLevelButton, GameScreen.WIDTH_IN_CELLS / 2, 15);
        }
        else
        {
            gameScreen.addObject(levelCompleteLabel, GameScreen.WIDTH_IN_CELLS / 2, 7);
            gameScreen.addObject(nextLevelButton, GameScreen.WIDTH_IN_CELLS / 2, 11);
            gameScreen.addObject(retryLevelButton, GameScreen.WIDTH_IN_CELLS / 2, 13);
        }
    }
    ...
    private void initHighscore()
    {
        ...
        gameScreen.addObject(highscoreGoldLabel, GameScreen.WIDTH_IN_CELLS / 2, 6);

        Label highscoreSilverLabel = new Label(second, 400, 72, GameScreen.FONT_L);
        ...
        gameScreen.addObject(highscoreSilverLabel, GameScreen.WIDTH_IN_CELLS / 2, 8);

        Label highscoreBronzeLabel = new Label(third, 400, 40, GameScreen.FONT_L);
        ...
        gameScreen.addObject(highscoreBronzeLabel, GameScreen.WIDTH_IN_CELLS / 2, 10);
        
        if (gameScreen.isHighscoreReadOnly())
        {
            Label highscoreReadOnlyLabel = new Label("You must be logged in to enter your Highscore!", 400, 16, GameScreen.FONT_S_BOLD);
            ...
            gameScreen.addObject(highscoreReadOnlyLabel, GameScreen.WIDTH_IN_CELLS / 2, 11);            
        }
    }
    ...
    public void act()
    {
        ...
        if (nextLevelButton.wasClicked())
        {
            if (! gameScreen.isGameComplete())
            {
                //  go to next level
                gameScreen.setCurrentLevelNumber(gameScreen.getCurrentLevelNumber() + 1);
                gameScreen.setState(gameScreen.getLevelSplashState());
            }
            else
            {
