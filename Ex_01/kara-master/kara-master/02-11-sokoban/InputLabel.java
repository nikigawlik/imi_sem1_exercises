...
public class InputLabel  extends Label
{
     ...
     */
    public InputLabel(String text, int width, int height, Font font)
    {
     ...
     * "space" and "backspace" can optionally be supported.
     */
    public void handleKeyPress(String key) 
    {
        if (key != null)
        {
            if ((maxLength == -1 || getText().length() < maxLength) && key.length() == 1) {
                setText(getText() + key);
                textChanged = true;
            }
            ...
            else if (allowSpace && (maxLength == -1 || getText().length() < maxLength) && key.equals("space")) {
                setText(getText() + " ");
            }
        }
    }
